  ...
  const searchOnline = async () => {
    ...
    try {
      const [archive, catalog] = await Promise.all([
        ...
        fetch(`/api/music/catalog?q=${encodeURIComponent(query)}`).then((response) => response.ok ? response.json() as Promise<Track[]> : [])
      ]);
      setOnlineTracks(archive);
      setCatalogTracks(catalog);
    } catch { setOnlineError("Online search is unavailable right now."); } finally { setOnlineLoading(false); }
  };
  ...
  return (
    <main className="grain min-h-screen bg-paper pb-36">
      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-10">
        ...
        {view === "search" && <section className="rise pt-10"><PageTitle title="Find a feeling" detail="Search your local collection or online catalog" /><div className="relative mb-3 mt-8"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-ink/45" size={20} /><input onKeyDown={(e) => { if (e.key === "Enter") void searchOnline(); }} autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tracks, artists, albums" className="w-full rounded-2xl border border-ink/15 bg-white/60 py-4 pl-12 pr-4 outline-none ring-lime focus:ring-2" /></div><button onClick={() => void searchOnline()} disabled={onlineLoading || !query.trim()} className="mb-8 rounded-full bg-ink px-4 py-2 text-sm font-bold text-paper disabled:opacity-40">{onlineLoading ? "Searching..." : "Search online"}</button><TrackRow tracks={filtered} current={current} onPlay={play} onFavorite={favorite} empty="Nothing here yet. Try a different search." />{onlineError && <p className="mt-6 text-sm text-ember">{onlineError}</p>}{catalogTracks.length > 0 && <div className="mt-12 border-t border-ink/15 pt-5"><h2 className="mb-3 font-display text-2xl font-bold">PYLA catalog</h2><TrackRow tracks={catalogTracks} current={current} onPlay={play} onFavorite={favorite} /></div>}{onlineTracks.length > 0 && <div className="mt-12 border-t border-ink/15 pt-5"><div className="mb-3 flex items-center justify-between"><h2 className="font-display text-2xl font-bold">Internet Archive</h2><span className="text-xs text-ink/45">Open-license sources vary</span></div><TrackRow tracks={onlineTracks} current={current} onPlay={play} onFavorite={favorite} /></div>}</section>}
