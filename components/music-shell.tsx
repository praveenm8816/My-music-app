...
export default function MusicShell() {
  ...
  const removeTrack = async (track: Track) => {
    ...
    if (current.id === track.id) { setPlaying(false); setCurrent(demoTracks[0]); }
  };
  ...
  return (
    <main className="grain min-h-screen bg-paper pb-36">
      ...
      <Player track={current} playing={playing} setPlaying={(value) => { if (!current.audioUrl) { setPlayerError("Choose a result with a playable stream."); return; } setPlaying(value); if (value) void audioRef.current?.play().catch(() => setPlaying(false)); else audioRef.current?.pause(); }} onNext={() => play(tracks[(tracks.findIndex((t) => t.id === current.id) + 1) % tracks.length])} onExpand={() => setExpandedPlayer(true)} />
